...
Given the following user request, write a strict structural specification
for the {artifact_type} that needs to be built.
...
Do not write the actual configuration file yet.
Write the specification (spec.md). Be extremely concise (max 150 words).

USER REQUEST:
{user_request}
"""
        # Research uses default routing: Groq → Gemini → Cerebras
...
What are the latest best practices, security hardening patterns, and modern
API usage recommendations for building a {artifact_type} for this specific request?

Provide concise actionable bullet points (research_notes.md).
Be extremely concise (max 150 words). Do not generate the actual file.
